...
public class TileStorageProxy extends TileEntity
    implements IGridProxyable, IActionHost, IStorageMonitorable, ITileStorageMonitorable, ITileWithModularUI
// ,IConfigurableObject
{
    ...
    public void gridChanged() {
        try {
            ...
            IMEMonitor<IAEItemStack> items = this.gridProxy.getStorage()
                .getItemInventory();
            ...
            IMEMonitor<IAEFluidStack> fluids = this.gridProxy.getStorage()
                .getFluidInventory();
    ...
    public ModularWindow createWindow(UIBuildContext buildContext) {
        ...
        if (!fluid) {
            ...
            for (int i = 0; i < 36; i++) builder.widget(
                SlotWidget.phantom(handler, i)
                    .setPos(3 + (i % 9) * 18, 3 + (i / 9) * 18));

            ;

            if (!noAdvConfig) builder.widget(
